...
class Refinement(nn.Module):
    def __init__(self, nf_in, nf, pass_occ, pass_feats, max_data_size, truncation=3):
        ...
        self.linearsemantic = nn.Linear(nf, 41)
    ...
    def forward(self,x):
        ...
        # predict semantic
        semantic = self.linearsemantic(x)
        ...
        if self.pass_feats and self.pass_occ:
            ...
            feats = out[mask]
        return [locs, feats], [locs_unfilt, out], [locs_unfilt,semantic]
...
class GenModel(nn.Module):
    ...
    def update_sizes(self, input_max_dim, refine_max_dim):
        ...
        input_max_dim = torch.from_numpy(input_max_dim)
        refine_max_dim = torch.from_numpy(refine_max_dim)
        for k in range(3):
    ...
    def forward(self, x, loss_weights):
        ...
        for h in range(len(self.refinement)):
            if loss_weights[h+1] > 0:
                ...
                x_sparse, occ, semantic = self.refinement[h](x_sparse)
                outputs.append(occ)
                outputs_semantic.append(semantic)
                #print('[model] refine(%d) x_sparse' % h, x_sparse[0].shape, torch.min(x_sparse[0]).item(), torch.max(x_sparse[0]).item())
            else:
                outputs.append([[],[]])
                outputs_semantic.append([[],[]])
        # surface prediction
        locs = x_sparse[0]
        if self.PRED_SURF and loss_weights[-1] > 0:
            ...
            # raw_input('sdlfkj')
            # #DEBUG SANITY - check batching same
            return [locs, x_sparse], outputs, outputs_semantic
        return [[],[]], outputs, outputs_semantic
