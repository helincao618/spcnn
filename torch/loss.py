...
def compute_sce_sparse_dense(sparse_pred_locs, sparse_pred_label, dense_semantic_block, batched=True):
    dims = dense_semantic_block.shape[1:]
    loss = 0.0 if batched else np.zeros(dense_semantic_block.shape[0], dtype=np.float32)
    predvalues = sparse_pred_label
    flatlocs = sparse_pred_locs[:,3]*dims[0]*dims[1]*dims[2] + sparse_pred_locs[:,0]*dims[1]*dims[2] + sparse_pred_locs[:,1]*dims[2] + sparse_pred_locs[:,2]
    tgtvalues = dense_semantic_block.view(-1)[flatlocs].long()
    CEloss = torch.nn.CrossEntropyLoss()
    loss = CEloss(predvalues, tgtvalues)
    return loss
...
# hierarchical loss 
def compute_loss(output_sdf, output_occs, target_for_sdf, target_for_occs, target_for_hier, outputs_semantic, semantic_target, semantic_hierarchy, loss_weights, truncation, use_log_transform=True, weight_missing_geo=1, input_locs=None, use_loss_masking=True, known=None, batched=True):
    ...
    losses_semantic = [] if batched else [[] for i in range(len(output_occs) + 1)]
    ...
    # loss semantic
    semantic_hierarchy.append(semantic_target)
    for h in range(len(outputs_semantic)):
        if len(outputs_semantic[h][0]) == 0 or loss_weights[h+1] == 0:
            if batched:
                losses_semantic.append(-1)
            else:
                losses_semantic[h].extend([-1] * batch_size)
            continue
