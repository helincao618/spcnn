...
def collate_train(batch):
    ...
    semantic_targets = torch.stack([x['semantic_target'] for x in batch])
    semantic_hierarchy = None
    if batch[0]['semantic_hierarchy'] is not None:
        semantic_hierarchy = [None]*len(batch[0]['semantic_hierarchy'])
        for h in range(len(batch[0]['semantic_hierarchy'])):
            semantic_hierarchy[h] = torch.stack([x['semantic_hierarchy'][h] for x in batch])
...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, files, semantic_files, input_dim, truncation, num_hierarchy_levels, max_input_height, num_overfit=0, target_path=''):
        ...
        if num_overfit > 0:
            ...
            self.semantic_files = self.semantic_files * num_repeat
    ...
    def __getitem__(self, idx):
        ...
        if self.is_chunks:
            ...
            label, hierarchy_level0, hierarchy_level1, hierarchy_level2 = data_util.load_semantic_file(semantic_file)
            label = torch.from_numpy(label)
            semantic_hierarchy = [torch.from_numpy(hierarchy_level0), torch.from_numpy(hierarchy_level1), torch.from_numpy(hierarchy_level2)]
