...
parser.add_argument('--input_dim', type=tuple, default=(128, 64, 64), help='voxel dim.')
...
# data files
train_files, val_files, semantic_train_files, semantic_val_files = data_util.get_train_files(args.data_path, args.semantic_data_path, args.train_file_list, args.val_file_list)
_OVERFIT = False
...
def print_log_info(epoch, iter, mean_train_losses, mean_losses_semantic, mean_train_l1pred, mean_train_l1tgt, mean_train_ious, mean_val_losses, mean_val_losses_semantic, mean_val_l1pred, mean_val_l1tgt, mean_val_ious, time, log):
    ...
    for h in range(len(mean_train_losses)):
        ...
        if log is None:
            ...
            splitters.append(',')
    values.extend(mean_losses_semantic)
    for h in range(len(mean_losses_semantic)):
        id = str(h+1)
        if log is None:
            splitters.append(' loss_train_semantic(' + id + '): ')
        else:
            splitters.append(',')
    ...
    if mean_val_losses is not None:
        ...
        for h in range(len(mean_val_losses)):
            ...
            if log is None:
                ...
                splitters.append(',')
        values.extend(mean_val_losses_semantic)
        for h in range(len(mean_val_losses_semantic)):
            id = str(h+1)
            if log is None:
                splitters.append(' loss_val_semantic(' + id + '): ')
            else:
                splitters.append(',')
...
def print_log(log, epoch, iter, train_losses, losses_semantic, train_l1preds, train_l1tgts, train_ious, val_losses, val_losses_semantic, val_l1preds, val_l1tgts, val_ious, time):
    train_losses = np.array(train_losses)
    train_l1preds = np.array(train_l1preds)
    train_l1tgts = np.array(train_l1tgts)
    train_ious = np.array(train_ious)
    losses_semantic = np.array(losses_semantic)
    mean_train_losses = [(-1 if np.all(x < 0) else np.mean(x[x >= 0])) for x in train_losses]
    mean_losses_semantic = [(-1 if np.all(x < 0) else np.mean(x[x >= 0])) for x in losses_semantic]
    ...
    if val_losses:
        ...
        print_log_info(epoch, iter, mean_train_losses, mean_losses_semantic, mean_train_l1pred, mean_train_l1tgt, mean_train_ious, mean_val_losses, val_losses_semantic, mean_val_l1pred, mean_val_l1tgt, mean_val_ious, time, None)
        print_log_info(epoch, iter, mean_train_losses, mean_losses_semantic, mean_train_l1pred, mean_train_l1tgt, mean_train_ious, mean_val_losses, val_losses_semantic, mean_val_l1pred, mean_val_l1tgt, mean_val_ious, time, log)
    else:
        print_log_info(epoch, iter, mean_train_losses, mean_losses_semantic, mean_train_l1pred, mean_train_l1tgt, mean_train_ious, None, None, None, None, None, time, None)
        print_log_info(epoch, iter, mean_train_losses, mean_losses_semantic, mean_train_l1pred, mean_train_l1tgt, mean_train_ious, None, None, None, None, None, time, log)
    log.flush()
...
def train(epoch, iter, dataloader, log_file, output_save):
    ...
    for t, sample in enumerate(dataloader):
        ...
        semantic_target = sample['semantic_target']
        semantic_hierarchy = sample['semantic_hierarchy']
        for h in range(len(hierarchy)):
            hierarchy[h] = hierarchy[h].cuda()
        for h in range(len(semantic_hierarchy)):
            semantic_hierarchy[h] = semantic_hierarchy[h].cuda()
        ...
        loss, losses, losses_semantic = loss_util.compute_loss(output_sdf, output_occs, target_for_sdf, target_for_occs, target_for_hier, outputs_semantic, semantic_target, semantic_hierarchy, loss_weights, args.truncation, args.logweight_target_sdf, args.weight_missing_geo, inputs[0], args.use_loss_masking, known)
        loss.backward()
        optimizer.step()
        ...
        if iter % 20 == 0:
            ...
            print_log(log_file, epoch, iter, train_losses, losses_semantic, train_l1preds, train_l1tgts, train_ious, None, None, None, None, None, took)
        if iter % 2000 == 0:
...
def test(epoch, iter, loss_weights, dataloader, log_file, output_save):
    ...
    with torch.no_grad():
        for t, sample in enumerate(dataloader):
            ...
            output_sdf, output_occs, outputs_semantic = model(inputs, loss_weights)
            loss, losses, val_losses_semantic = loss_util.compute_loss(output_sdf, output_occs, target_for_sdf, target_for_occs, target_for_hier, outputs_semantic, semantic_target, semantic_hierarchy, loss_weights, args.truncation, args.logweight_target_sdf, args.weight_missing_geo, inputs[0], args.use_loss_masking, known)
...
def main():
    ...
    headers.extend(['train_loss(sdf)'])
    headers.extend(['loss_train_semantic(1)','loss_train_semantic(2)','loss_train_semantic(3)'])
    headers.extend(['train_l1-pred', 'train_l1-tgt'])
    for h in range(args.num_hierarchy_levels):
    ...
    if has_val:
        ...
        headers.extend(['val_loss(sdf)'])
        headers.extend(['loss_val_semantic(1)','loss_val_semantic(2)','loss_val_semantic(3)'])
        headers.extend(['val_l1-pred', 'val_l1-tgt'])
        for h in range(args.num_hierarchy_levels):
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        train_losses, losses_semantic, train_l1preds, train_l1tgts, train_ious, iter, loss_weights = train(epoch, iter, train_dataloader, log_file, output_save=(epoch % args.save_epoch == 0))
        if has_val:
            val_losses, val_losses_semantic, val_l1preds, val_l1tgts, val_ious = test(epoch, iter, loss_weights, val_dataloader, log_file_val, output_save=(epoch % args.save_epoch == 0))

        took = time.time() - start
        if has_val:
            print_log(log_file_val, epoch, iter, train_losses, losses_semantic, train_l1preds, train_l1tgts, train_ious, val_losses, val_losses_semantic, val_l1preds, val_l1tgts, val_ious, took)
        else:
            print_log(log_file, epoch, iter, train_losses, losses_semantic, train_l1preds, train_l1tgts, train_ious, None, None, None, None, None, took)
