...
def load_semantic_file(h5_file_path):
    f = h5py.File(h5_file_path, 'r')
    label = f['label'][:]
    hierarchy_level0 = f['hierarchy_level0'][:]
    hierarchy_level1 = f['hierarchy_level1'][:]
    hierarchy_level2 = f['hierarchy_level2'][:]
    return label, hierarchy_level0, hierarchy_level1, hierarchy_level2

def load_semantic_scene(h5_file_path):
    f = h5py.File(h5_file_path, 'r')
    label = f['label'][:]
    world2grid = f['world2grid'][:]
    return label, world2grid


def get_train_files(data_path, semantic_data_path, train_file_list, val_file_list):
    train_names = open(train_file_list).read().splitlines()
    if not '.' in train_names[0]:
        train_names = [name + '__0__.sdf' for name in train_names]
    train_files = [os.path.join(data_path, f) for f in train_names]
    semantic_train_files = [os.path.join(semantic_data_path, f.rstrip('.sdfs'))+'.h5' for f in train_names]
    val_files = []
    semantic_val_files = []
    if val_file_list:
